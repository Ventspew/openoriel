...
- Force `background.persistent: false` (on iOS, omitting the key for MV2 page/scripts defaults to persistent and fails install with “Invalid `persistent` manifest entry”); prefer `service_worker` when both shapes exist
